...
\author{P Deepak Reddy \\EE23BTECH11047	}
...
\begin{align}
    ...
    j&=8
\end{align}
 substituting $j =8$ gives the passband
...
\begin{align}
    \omega_{p1} &= 2\pi\frac{F_{p1}}{F_s} = 0.37 \pi\\
    \omega_{p2} &= 2\pi\frac{F_{p2}}{F_s}  = 0.42\pi
\end{align}
...
\begin{align}
    ...
    F_{s2} &= 10+0.3 = 10.3 \text{KHz}
\end{align}
\begin{align}
    \omega_{s1} &= 2\pi\frac{F_{s1}}{F_s} = 0.3542 \pi\\
     \omega_{s2} &= 2\pi\frac{F_{s2}}{F_s} = 0.4292 \pi\\
\end{align}
\end{enumerate}
...
$\Omega_{p1} = 0.6569$, $\Omega_{p2} = 0.7757$ and $\Omega_{s1} = 0.6219$, $\Omega_{s2} = 0.7990$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.7138$ and $B = \Omega_{p2} - \Omega_{p1} = 0.1188$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = -1.6614\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = 1.3579
\end{align}And we choose the minimum of these two stopband edges
\begin{align}
    \Omega_{Ls} = \mbox{min}(\vert \Omega_{Ls_1}\vert,\vert \Omega_{Ls_2}\vert) = 1.3579
\end{align}
...
\end{align}
Imposing the band restrictions on (17) \\
\begin{align}
...
\newpage
The below code plots (17) for different values of $\epsilon$ .
\begin{lstlisting}
https://github.com/Deepakreddy1510/Filter_Design/blob/main/codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\begin{lstlisting}
https://github.com/Deepakreddy1510/Filter_Design/blob/main/codes/sk_gen.c
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/Deepakreddy1510/Filter_Design/blob/main/codes/pole_zero_plt.py
\end{lstlisting}
\newpage
\begin{figure}[H]
\centering
...
\end{align}
Substituting respective values in \eqref{eq:Gain_eq_LP} we get $G_{LP}=0.2551$
\begin{align}
     H_{a,LP}(s_L) &= \frac{0.2551}{\brak{s_L-s_5}\brak{s_L-s_6}\brak{s_L-s_7}\brak{s_L-s_8}}\\
     &= \frac{0.2551}{s_L^4 + 0.9764s_L^3 + 1.4767s_L^2+0.7675s_L + 0.2841}\label{eq:design}
\end{align}
...
Plot fot Figure 3
\begin{lstlisting}
https://github.com/Deepakreddy1510/Filter_Design/blob/main/codes/plot2.py
\end{lstlisting}
...
\begin{align}
   H_{a,BP}(s) = \frac{5.6489\times 10^{-5}s^4}{s^8 + 0.1160s^7 + 2.0588s^6 + 0.1786s^5 + 1.5790s^4 + 0.0909s^3 + 0.5345s^2 + 0.0153s + 0.0674}\label{eq:magnitude_bandpass_sdom}
\end{align}
...
\begin{lstlisting}
https://github.com/Deepakreddy1510/Filter_Design/blob/main/codes/plot3.py
\end{lstlisting}
\end{enumerate}
...
\end{align}
Substituting $s=\frac{1-z^{-1}}{1+z^{-1}}$ in (43) and calculating expression using a python code we get :
\begin{align}
    H_{d,BP}(z) &= \frac{G\brak{1 - 4z^{-2} + 6z^{-4} - 4z^{-6} + z^{-8}}}{5.64  -14.33z^{-1} + 35.24z^{-2}  -47.01z^{-3}+  58.26z^{-4}
  -45.24z^{-5}+   32.64z^{-6}  -12.78z^{-7} +   4.84z^{-8}}
\end{align}

where $G=5.6489\times 10^{-5}$ \\   
\includegraphics[width=1.0\columnwidth]{figs/Digital_BPF.png}
...
The lowpass filter has a passband frequency $\omega_l$ and transition band $\Delta \omega = 2\pi \frac{\Delta F}{F_s} = 0.0392\pi$.
...
\end{cases} 
\end{align}
From eq(48) we conclude that $h\brak{n}$ for an ideal Low Pass Filter is not causal and can neither be made causal by introducing a finite delay. And $h\brak{n}$ do not converge and hence the system is unstable.
...
\end{figure}
\begin{lstlisting}
https://github.com/Deepakreddy1510/Filter_Design/blob/main/codes/plot4.py
\end{lstlisting}
...
\begin{align}
     \frac{\sin(\omega_{p2} n)}{n\pi} -\frac{\sin\brak{\omega_{p1}n}}{n\pi} &= \frac{2\cos{\brak{\frac{\omega_{p2}n+\omega_{p1}n}{2}}}\sin{\brak{\frac{\omega_{p2}n-\omega_{p1}n}{2}}}}{n\pi}\\
            &= \frac{2\cos{\brak{0.395n\pi}}\sin{\brak{0.025n\pi}}}{n\pi}
\end{align}
Multipying by window function we get :
\begin{align}
    h_{BP}\brak{n} = 
\begin{cases} 
   \frac{2\cos{\brak{0.395n\pi}}\sin{\brak{0.025n\pi}}}{n\pi},  & \text{for } -48\leq n \leq 48 \\
    0 &\text{otherwise}
...
\end{figure}
\begin{lstlisting}
https://github.com/Deepakreddy1510/Filter_Design/blob/main/codes/plot5.py
\end{lstlisting}
